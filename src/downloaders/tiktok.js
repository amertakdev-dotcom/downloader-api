...
async function getTikTokInfo(url) {
  ...
  try {
    ...
    if (tikwm && tikwm.code === 0 && tikwm.data) {
      ...
      if (!result.data.thumbnail) result.data.thumbnail = d.cover || d.origin_cover || null;
      result.data.duration = d.duration || null;
      result.data.playCount = d.play || null;
      ...
      // Video downloads
      if (d.play) result.data.downloads.noWatermark = d.play;
      if (d.wmplay) result.data.downloads.withWatermark = d.wmplay;
      if (d.hdplay) result.data.downloads.hdNoWatermark = d.hdplay;
      if (d.music) result.data.downloads.audio = d.music;
      if (d.music_info) {
        result.data.music = {
