...
async function getFromSnapTik(url) {
  ...
  // muscdn.com links are TikTok's *music/audio* CDN, not video — they must
  // be kept separate from the video links, otherwise an audio-only URL can
  // end up mistakenly assigned as the "withWatermark" video download.
  const videoUrls = [];
  const videoPatterns = [
    /href="(https:\/\/[^"]+\.mp4[^"]*)"/g,
    /src="(https:\/\/[^"]+\.mp4[^"]*)"/g,
  ...
  const audioUrls = [];
  const audioPattern = /"(https:\/\/v\d+\.muscdn\.com[^"]*)"/g;
  let am;
  while ((am = audioPattern.exec(html)) !== null) {
    const u = am[1].replace(/\\u0026/g, "&").replace(/\\/g, "");
...
async function getFromSssTik(url) {
  ...
  // ssstik also exposes a separate "download MP3" link for the track audio.
  const audioUrls = [];
  const audioPattern = /href="(https:\/\/[^"]+\.mp3[^"]*)"/g;
  let am;
  while ((am = audioPattern.exec(html)) !== null) {
    const u = am[1].replace(/\\u0026/g, "&").replace(/\\/g, "");
    if (u && !audioUrls.includes(u)) audioUrls.push(u);
  ...
  return { videoUrls: urls, audioUrls };
}
...
async function getTikTokInfo(url) {
  ...
  try {
    const { videoUrls, audioUrls } = await getFromSnapTik(resolvedUrl);
    if (videoUrls.length > 0 || audioUrls.length > 0) {
      if (videoUrls[0]) result.data.downloads.noWatermark = videoUrls[0];
      if (videoUrls[1]) result.data.downloads.withWatermark = videoUrls[1];
      if (audioUrls[0]) result.data.downloads.audio = audioUrls[0];
      return result;
    }
  } catch (e) {
  ...
  try {
    const { videoUrls: sssVideoUrls, audioUrls: sssAudioUrls } = await getFromSssTik(resolvedUrl);
    if (sssVideoUrls.length > 0 || sssAudioUrls.length > 0) {
      if (sssVideoUrls[0]) result.data.downloads.noWatermark = sssVideoUrls[0];
      if (sssVideoUrls[1]) result.data.downloads.withWatermark = sssVideoUrls[1];
      if (sssAudioUrls[0]) result.data.downloads.audio = sssAudioUrls[0];
      return result;
    }
  } catch (e) {
